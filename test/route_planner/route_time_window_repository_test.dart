...
void main() {
  ...
  test(
    'converts clock windows to minutes after departure in the request',
    () async {
      when(
        () => ai.optimize(any()),
      ).thenAnswer((_) async => _response(['a', 'b']));
      stubRoad(3);
      ...
      final sent =
          verify(() => ai.optimize(captureAny())).captured.single
              as RouteRequestModel;
      final json = sent.toJson();
      final deliveries = json['deliveries'] as List;

      final withWindow =
          deliveries.firstWhere((d) => d['address'] == 'a') as Map;
  ...
  test(
    'raises driver_hours so a late window stays inside the working day',
    () async {
  ...
  test(
    'flags a stop the solver silently dropped, and keeps it in the route',
    () async {
      ...
      expect(
        b.timeWindowMissed,
        isTrue,
        reason: 'dropped stop must be flagged',
      );
  ...
  test('does not flag a dropped stop the user set no window on', () async {
    // Same silent drop as above, but 'b' carries no availability at all.
    // The solver drops stops for reasons that have nothing to do with a
    // clock, and warning about a missed arrival time here would invent a
    // constraint the user never entered.
    when(() => ai.optimize(any())).thenAnswer((_) async => _response(['a']));
    stubRoad(3);
    ...
    final route = expectSuccess(result);
    final b = route.orderedPoints.firstWhere((p) => p.id == 'b');
    expect(b.timeWindowMissed, isFalse);
    expect(b.latenessMinutes, isNull);
    // Still kept — silence is about the warning, not about the stop.
    expect(route.orderedPoints.map((p) => p.id), contains('b'));
  });
  ...
  test(
    'holds the ETA until the window opens, and delays later stops',
    () async {
      when(
        () => ai.optimize(any()),
      ).thenAnswer((_) async => _response(['a', 'b']));
  ...
  test(
    'flags a stop the solver accepted but the road time overshoots',
    () async {
